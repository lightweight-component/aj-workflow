...
import type { WorkflowDefinition, WorkflowNode, WorkflowTransition } from "@/domain/workflow";
...
const byType: Record<string, FormField[]> = {
  task: [
    ...
    { name: "assignee", label: "参与者" },
    {
      ...
      label: "任务类型",
      options: [
        { value: "Major", label: "主办" },
        { value: "Aidant", label: "协办" },
      ],
    },
    {
      name: "performType",
      label: "参与类型",
      options: [
        { value: "ANY", label: "任一参与者" },
        { value: "ALL", label: "全部参与者" },
      ],
    },
  ...
  decision: [
    { name: "expr", label: "决策表达式" },
    { name: "handleClass", label: "决策处理类" },
  ],
  custom: [
    { name: "form", label: "表单 URL" },
    { name: "clazz", label: "执行类" },
    { name: "methodName", label: "方法名" },
    { name: "args", label: "参数变量" },
    { name: "var", label: "返回值变量" },
  ],
  subprocess: [
    { name: "processName", label: "子流程名称" },
    { name: "version", label: "子流程版本", type: "number" },
    { name: "form", label: "表单 URL" },
  ],
};

const visibleFields: ComputedRef<FormField[]> = computed(() => [
  ...commonFields,
...
const processFields: FormField[] = [
  ...
  { name: "displayName", label: "显示名称" },
  { name: "expireTime", label: "期望完成时间" },
  { name: "instanceUrl", label: "实例 URL" },
  { name: "instanceNoClass", label: "实例编号生成类" },
];

/** 通知父组件属性数据已经发生变化。 */
const changed = (): void => emit("change");

/** 读取当前节点的指定属性值。 */
const getProp = (name: string): string => node.value?.props[name]?.value ?? "";
...
/** 读取当前连线的指定属性值。 */
const getPathProp = (name: string): string => path.value?.props[name]?.value ?? "";

/** 更新当前连线的指定属性值。 */
const setPathProp = (name: string, value: string): void => {
  if (path.value) {
    path.value.props[name] = { value };
    changed();
  }
};
...
const removeField = (index: number): void => {
  node.value?.fields?.splice(index, 1);
  changed();
};
