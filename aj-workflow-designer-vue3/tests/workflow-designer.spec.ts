...
describe("WorkflowDesigner component", () => {
  ...
  it("scopes keyboard shortcuts to the focused designer instance", async () => {
    const first = mount(WorkflowDesigner, {
      props: { modelValue: cloneWorkflow(SAMPLE_WORKFLOW) },
      attachTo: document.body,
    });
    const second = mount(WorkflowDesigner, {
      props: { modelValue: cloneWorkflow(SAMPLE_WORKFLOW) },
      attachTo: document.body,
    });
    ...
    expect(
      (first.emitted("update:modelValue")![0][0] as WorkflowDefinition).states.apply,
    ).toBeUndefined();
    first.unmount();
    second.unmount();
  });
  ...
  it("treats external model replacement as a clean baseline", async () => {
    ...
    expect(
      (wrapper.vm as unknown as { getWorkflow(): WorkflowDefinition }).getWorkflow().props
        .displayName,
    ).toBe("外部载入");
  });
  ...
  it("does not allow the Start node to be copied", async () => {
    ...
    expect((wrapper.vm as unknown as { getSelection(): unknown }).getSelection()).toEqual({
      kind: "node",
      ref: "start1",
    });
  });
  ...
  it("rejects a malformed external model before rendering it", async () => {
    ...
    expect(
      (wrapper.vm as unknown as { getWorkflow(): WorkflowDefinition }).getWorkflow().states.bad,
    ).toBeUndefined();
  });
