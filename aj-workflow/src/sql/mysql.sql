CREATE TABLE IF NOT EXISTS `wf_process` (
  ...
  `instance_url` varchar(200) DEFAULT NULL COMMENT '实例 url',
...
CREATE TABLE IF NOT EXISTS `wf_order` (
  ...
  `process_id` varchar(100) NOT NULL COMMENT '流程定义 id',
  `priority` tinyint(2) DEFAULT NULL COMMENT '优先级',
  `parent_id` varchar(100) DEFAULT NULL COMMENT '父流程 id',
  `parent_node_name` varchar(100) DEFAULT NULL COMMENT '父流程依赖的节点名称',
  `expire_time` datetime DEFAULT NULL COMMENT '期望完成时间',
  `variable` varchar(2000) DEFAULT NULL COMMENT '附属变量 json 存储',
...
CREATE TABLE IF NOT EXISTS `wf_task` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键 id，自增',
  `order_id` varchar(100) NOT NULL COMMENT '流程实例 id',
  ...
  `action_url` varchar(200) DEFAULT NULL COMMENT '任务处理的 url',
  `parent_task_id` varchar(100) DEFAULT NULL COMMENT '父任务 id',
  `variable` varchar(2000) DEFAULT NULL COMMENT '附属变量 json 存储',
