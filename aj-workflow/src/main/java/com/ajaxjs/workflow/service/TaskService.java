...
//		orderService.getCompletion().accept(history, null);
    ...
    public Task withdrawTask(Long taskId, Long operator) {
        TaskHistory history = WfData.findTaskHistory(taskId);
        Objects.requireNonNull(history, "指定的历史任务[id=" + taskId + "]不存在");
    ...
    public Task rejectTask(ProcessModel model, Task currentTask) {
        ...
        TaskHistory history = WfData.findTaskHistory(parentTaskId);
        NodeModel parent = model.getNode(history.getName());

        if (!canRejected(current, parent))
            throw new WfException("无法驳回至上一步处理，请确认上一步骤并非 fork、join、subprocess 以及会签任务");
    ...
    public Task resume(Long taskId, Long operator) {
        TaskHistory histTask = WfData.findTaskHistory(taskId);
...
//				return true;
     ...
     * <p>
     * operator 操作人 id
     * actors   参与者列表 传递至该接口的实现类中的参与者都是为非空。确定的组集合[如操作人属于多个部门、拥有多个角色]
     * boolean 是否允许访问
     */
...
//			taskActor.setActorId(actorId);
    ...
    /**
     * 根据 task-model 指定的 assignee 属性，从 args 中取值将取到的值处理为 String[] 类型。
     *
     * @param actors 参与者对象
     * @return 参与者数组
     */
    private static Long[] getActors(Object actors) {
        ...
        if (actors instanceof String[]) {
            ...
            // Jackson 会把 string[] 转成 arraylist，此处增加 arraylist 的逻辑判断,by 红豆冰沙2014.11.21
            List<?> list = (List<?>) actors;
            results = new Long[list.size()];

            for (int i = 0; i < list.size(); i++)
                results[i] = Long.parseLong(list.get(i).toString());
        } else if (actors instanceof Long) {// 如果为Long类型，则返回1个元素的 String[]
            results = new Long[1];
            results[0] = (Long) actors;
        } else if (actors instanceof Integer) {// 如果为Integer类型，则返回1个元素的 String[]
            results = new Long[1];
            results[0] = Long.parseLong(String.valueOf(actors));
